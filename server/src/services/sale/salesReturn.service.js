...
// ✅ UOM helper – locks the “math in base, store {primary, base}” rule
...
async function createSalesReturn(payload) {
  ...
  try {
    ...
    let invoiceItemFactorMap = new Map(); // itemId -> factorToBase from invoice

    if (originalInvoice) {
      invoice = await SalesInvoice.findById(originalInvoice)
        .select(
          "salesRep invoiceNo totalValue totalReturnedValue totalBalanceValue items hasReturns returns invoiceDate customer branch status"
        )
        .session(session)
        .lean();
      ...
      // Build map: itemId -> factorToBase from original invoice items
    ...
    // ✅ SalesRep ownership
    const finalSalesRep = originalInvoice
      ? invoiceSalesRep
      : salesRep
      ? String(salesRep)
      : null;
    ...
    const items = (rawItems || []).map((line) => {
      ...
      const sellingPriceBase =
        rawSellingPriceBase == null ? null : toNumber(rawSellingPriceBase);
      const sellingPricePrimary =
        rawSellingPricePrimary == null ? null : toNumber(rawSellingPricePrimary);

      let factorToBase =
        invoiceItemFactorMap.get(itemIdStr) ?? line.factorToBase ?? 1;
      factorToBase = toNumber(factorToBase) || 1;

      // ---------- Validations ----------
      if (qtyReturnPrimary < 0 || qtyReturnBase < 0)
        throw new Error(`Invalid return quantity (negative) for item ${line.item}`);

      if (qtyReturnPrimary === 0 && qtyReturnBase === 0)
        throw new Error(`At least one return quantity must be > 0 for item ${line.item}`);

      if (qtyReturnBase > 0 && (!sellingPriceBase || sellingPriceBase <= 0))
        throw new Error(`Invalid base selling price for item ${line.item} (qtyReturnBase > 0)`);

      if (qtyReturnPrimary > 0 && (!sellingPricePrimary || sellingPricePrimary <= 0))
        throw new Error(`Invalid primary selling price for item ${line.item} (qtyReturnPrimary > 0)`);
      ...
      if (
        (qtyReturnBase > 0 && sellingPriceBase != null && discountPerUnit > sellingPriceBase) ||
        (qtyReturnPrimary > 0 && sellingPricePrimary != null && discountPerUnit > sellingPricePrimary)
      ) throw new Error(`Discount cannot exceed selling price for item ${line.item}`);

      const totalSellingValue =
        qtyReturnPrimary * (sellingPricePrimary ?? 0) +
        qtyReturnBase * (sellingPriceBase ?? 0);

      return {
        item: itemId,
        qtyReturnPrimary: Math.abs(qtyReturnPrimary),
        qtyReturnBase: Math.abs(qtyReturnBase),
        sellingPriceBase,
        sellingPricePrimary,
        factorToBase,
        discountPerUnit,
    ...
    // ---------- STRICT VALIDATION: Cannot exceed remaining invoice qty ----------
    // ---------- STRICT VALIDATION: Cannot exceed remaining invoice qty ----------
    if (originalInvoice) {
      const invoiceWithRemaining = await getInvoice(originalInvoice, {
        salesRep: finalSalesRep,
      });
      if (!invoiceWithRemaining) throw new Error("Original invoice not found");

      for (const line of items) {
        const remainingLine = (invoiceWithRemaining.remainingItems || []).find(
          (i) => String(i.item) === String(line.item)
        );

        if (!remainingLine)
          throw new Error(`Item ${line.item} not found in original invoice`);

        const factorToBase = toNumber(line.factorToBase) || 1;

        // 🔥 Convert return to total base
        const returnedTotalBase =
          toNumber(line.qtyReturnPrimary) * factorToBase +
          toNumber(line.qtyReturnBase);

        // 🔥 Convert remaining to total base
        const remainingTotalBase =
          toNumber(remainingLine.remainingPrimaryQty) * factorToBase +
          toNumber(remainingLine.remainingBaseQty);

        if (returnedTotalBase <= 0)
          throw new Error(
            `Returned quantity must be greater than zero for item ${line.item}`
          );

        if (returnedTotalBase > remainingTotalBase)
          throw new Error(
            `Return quantity exceeds remaining quantity for item ${line.item}`
          );
      }
    }
...
//--------------------------------------------------------
// APPROVE SALES RETURN (Admin/DataEntry only)
//--------------------------------------------------------
async function approveSalesReturn(id, userId) {
  ...
  try {
    ...
    if (doc.status !== "waiting_for_approval")
      throw new Error("SalesReturn not in waiting_for_approval status");
    ...
    for (const line of doc.items || []) {
      ...
      if (salesRepId) {
        ...
        if (!salesRepStock)
          throw Object.assign(new Error(`SalesRep stock not found for item ${line.item}`), {
            status: 400,
    ...
    if (doc.originalInvoice) {
      ...
      // STRICT validation: cannot exceed invoice
      // STRICT VALIDATION (BASE TRUTH — ENTERPRISE SAFE)
      for (const returnLine of doc.items || []) {
        ...
        if (!invoiceLine)
          throw new Error(`Returned item not found in original invoice: ${returnLine.item}`);

        const factorToBase = toNumber(invoiceLine.factorToBase || 1);

        const invoiceTotalBase =
          toNumber(invoiceLine.primaryQty  || 0) * factorToBase +
          toNumber(invoiceLine.baseQty  || 0);

        let alreadyReturnedBase = 0;

        for (const prevReturn of invoiceDoc.returns || []) {
          // 🔥 Ignore this current return if already linked
        ...
        if (currentBaseReturn <= 0)
          throw new Error(`Invalid return quantity for item ${returnLine.item}`);

        if (alreadyReturnedBase + currentBaseReturn > invoiceTotalBase)
          throw new Error(
            `Return exceeds invoice quantity for item ${returnLine.item}`
          );
      }


      // Update invoice returns
