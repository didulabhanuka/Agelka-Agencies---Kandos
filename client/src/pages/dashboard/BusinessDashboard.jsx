...
const BusinessDashboard = ({ salesRepId }) => {
  ...
  // 🔁 Aggregate branch totals (item count + financials)
  const branchTotals = useMemo(() => {
    return branches.reduce(
      (acc, b) => {
        acc.totalRevenue += Number(b.totalRevenue || 0);
        acc.totalCost += Number(b.totalCost || 0);
        acc.totalProfit += Number(b.profit || 0);
        acc.totalItemCountSold += Number(b.itemCountSold || 0);
        return acc;
      },
      {
        totalRevenue: 0,
    ...
    <div className="container-fluid py-4 px-5">
      ...
      <div className="summary-grid">
        ...
        <div className="summary-card">
          ...
          <div className="summary-value">{formatMoney(summary.totalRevenue)}</div>
        ...
        <div className="summary-card">
          ...
          <div className="summary-value">{formatMoney(summary.totalCost)}</div>
          <div className="summary-sub">
        ...
        <div className="summary-card orange">
          ...
          <div className="summary-value">{formatPercent(summary.margin)}</div>
          <div className="summary-sub">Profit as a percentage of revenue.</div>
          <div className="summary-period">Period: {periodLabel}</div>
        </div>
      </div>
                    ...
                    {branches.length === 0 ? (
                      ...
                      branches.map((b) => (
                        <tr key={b.branchId}>
                          ...
                          <td className="text-end">{formatMoney(b.totalCost)}</td>
                          <td className="text-end">{formatMoney(b.profit)}</td>
                          <td className="text-end">{formatPercent(b.margin)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
                    ...
                    <tfoot>
                      <tr>
                        ...
                        <th className="text-end">
                          {formatCount(branchTotals.totalItemCountSold)}
                        </th>
                        <th className="text-end">
                    ...
                    {items.length === 0 ? (
                      ...
                      items.map((it) => (
                        <tr key={`${it.itemId}_${it.branchId}`}>
                          ...
                          <td className="text-end">{formatMoney(it.profit)}</td>
                          <td className="text-end">{formatPercent(it.margin)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
