...
const StockDashboard = ({ setActiveView, salesRepId }) => {
  ...
  periodItemCount: 0,
  periodBranchCount: 0,
  onHand: { qty: { baseQty: 0, primaryQty: 0 }, stockValue: 0, itemCount: 0, branchCount: 0 }, // Default values
...
const [outOfStockCount, setOutOfStockCount] = useState(0);
  ...
  const loadSnapshot = async (from, to) => {
    try {
      ...
      const [data, itemsRes, stockDetailsRes] = await Promise.all([
        getStockSnapshot(params),
        getItems({}), // you can filter here if needed
      listSalesRepStockDetails(params), // Fetch stock details for low stock and out of stock
      ]);

    // Count low_stock and out_of_stock items
    const lowStockItems = stockDetailsRes.filter(item => item.stockStatus === 'low_stock');
    const outOfStockItems = stockDetailsRes.filter(item => item.stockStatus === 'out_of_stock');
    ...
    setOutOfStockCount(outOfStockItems.length);
      ...
      const {
        ...
        periodItemCount = 0,
        periodBranchCount = 0,
        onHand = { qty: { baseQty: 0, primaryQty: 0 }, stockValue: 0, itemCount: 0, branchCount: 0 }, // Default fallback
        itemsMovement = [],
        branchesMovement = [],
        onHandRows = [],
        transactionTypeTotals = {},
      } = data || {}; // Safe destructuring with fallback values
      ...
      setSummary({
        ...
        periodItemCount,
        periodBranchCount,
        onHand, // Set onHand from response
        transactionTypeTotals,
      });

      setItemsMovement(Array.isArray(itemsMovement) ? itemsMovement : []);
      setBranchesMovement(Array.isArray(branchesMovement) ? branchesMovement : []);
      setOnHandRows(Array.isArray(onHandRows) ? onHandRows : []);
...
<div className="summary-card">
  ...
  <div className="summary-value">
    {formatNumber(summary.onHand.itemCount)} {/* Show itemCount */}
  ...
  <div className="summary-sub">
    Approx. value {formatMoney(summary.onHand.stockValue)} in stock.
  </div>

  <div className="summary-sub">
    {formatNumber(summary.onHand.itemCount)} items ·{" "}
    {formatNumber(summary.onHand.branchCount)} branches
  </div>
...
<div className="summary-card orange">
  ...
  <div className="summary-value">
    {lowStockCount + outOfStockCount} {/* Total stock alerts */}
  </div>
