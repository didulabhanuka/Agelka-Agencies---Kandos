...
const SalesReturnCreateModal = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setBranches(res[0]?.data || res[0] || []);
        setCustomers(res[1] || []);
        setSalesReps(isAdminOrDataEntry ? res[2]?.data || res[2] || [] : []);
      } catch (err) {
  ...
  // ── Map an invoice's items into form rows ───────────────────────────────────
  const mapInvoiceToFormItems = (invoice, returnedMap, existingQtyMap = null) => {
      ...
      <Modal show={show} onHide={onClose} centered backdrop="static" dialogClassName="sales-return-create-modal">
        ...
        <Modal.Body>
          ...
          <div className="text-end mt-4">
            <Button className="action-btn-modal" onClick={onClose}>Close</Button>
          </div>
        </Modal.Body>
      </Modal>
    );
  ...
  const handleSubmit = async (e) => {
    ...
    if (hasInvalidLines) {
      toast.warning("Please ensure each non-zero return quantity has a selling price for that UOM.");
      return;
    }
    ...
    const payloadItems = validLines.map((i) => ({
      ...
      sellingPricePrimary:
        Number(i.sellingPricePrimary || 0) > 0 ? Number(i.sellingPricePrimary) : null,
      discountPerUnit:
        i.discountPerUnit === "" || i.discountPerUnit == null || Number(i.discountPerUnit) === 0
          ? null
  ...
  // ── Derived labels ──────────────────────────────────────────────────────────
  const titleText    = isView ? "View Sales Return" : isEdit ? "Edit Sales Return" : "Create Sales Return";
  ...
  return (
    <>
      <style>{`
        .sales-return-create-modal { max-width: 96vw !important; width: 96vw; }
        .sales-return-create-modal .modal-dialog { max-width: 96vw !important; width: 96vw !important; }
        .sales-return-create-modal .modal-content { height: 92vh; border-radius: 16px; overflow: hidden; }
        ...
        .invoice-section-card {
          ...
          padding: 14px; box-shadow: 0 2px 8px rgba(16,24,40,0.04);
        }
        .invoice-section-title {
          font-size: 0.86rem; font-weight: 700; color: #475467;
          margin-bottom: 10px; display: flex; align-items: center; gap: 8px;
        }
        .return-table-wrap {
          border: 1px solid #e9edf3; border-radius: 12px; overflow: auto;
          max-height: 43vh; background: #fff;
        }
        .return-table { width: 100%; min-width: 1320px; border-collapse: separate; border-spacing: 0; }
        .return-table thead th {
          position: sticky; top: 0; z-index: 5; background: #f8fafc;
          border-bottom: 1px solid #e9edf3; color: #475467; font-size: 0.78rem;
          font-weight: 700; text-transform: uppercase; letter-spacing: .02em;
          padding: 10px 8px; white-space: nowrap;
        }
        .return-table tbody td {
          border-bottom: 1px solid #f1f3f7; padding: 8px; vertical-align: top; background: #fff;
        }
        .return-table tbody tr:hover td { background: #faf7ff; }
        .sr-row-disabled td { background: #fafafa !important; opacity: 0.8; }
        .invoice-muted-chip {
          display: inline-flex; align-items: center; background: #f3f4f6;
          color: #6b7280; border: 1px solid #e5e7eb; border-radius: 999px;
          padding: 2px 8px; font-size: 11px; margin-top: 5px; margin-right: 6px;
        }
        .invoice-warning-chip {
          ...
          padding: 2px 8px; font-size: 11px; margin-top: 5px;
        }
        .invoice-summary-bar {
          position: sticky; bottom: 0; z-index: 8;
          background: rgba(255,255,255,0.95); backdrop-filter: blur(6px);
          border: 1px solid #e9edf3; border-radius: 12px; padding: 10px 12px; margin-top: 10px;
        }
        .invoice-footer-bar {
        ...
        .sr-stat-inline {
          ...
          border-radius: 999px; border: 1px solid #e5e7eb; background: #fff; font-size: 12px; color: #374151;
        }
        .sr-link-btn {
          border: 1px solid #dbeafe; background: #eff6ff; color: #1d4ed8; border-radius: 10px;
          padding: 6px 10px; font-size: 12px; font-weight: 600;
          display: inline-flex; align-items: center; gap: 6px;
        }
        .sr-link-btn:hover { background: #dbeafe; }
        .edit-mode-notice {
          ...
          <form onSubmit={handleSubmit}>
            ...
            {isEdit && (
              <div className="edit-mode-notice">
                ...
                You are editing an existing sales return. Only quantities and remarks can be adjusted.
                The linked invoice cannot be changed.
              </div>
            )}
              ...
              <div className="row g-3">
                ...
                <div className="col-md-8">
                  <div className="form-floating react-select-floating">
                    <Select
                      classNamePrefix="react-select"
                      isDisabled={invoiceSelectDisabled}
                      isClearable={isCreate}
                      options={invoices.map((inv) => ({
                        label: `${inv.invoiceNo} — ${formatDate(inv.invoiceDate)} — ${formatCurrency(inv.totalValue)}`,
                        value: inv._id,
                      }))}
                      value={
                      ...
                      onChange={(opt) => isCreate && loadInvoiceAsSource(opt ? opt.value : "")}
                      styles={selectStyles}
                      menuPortalTarget={document.body}
                      placeholder=""
                    />
                ...
                <table className="return-table">
                  <thead>
                    <tr>
                      ...
                      <th className="text-end" style={{ minWidth: 130 }}>Return Qty (Base)</th>
                      <th className="text-end" style={{ minWidth: 140 }}>Return Qty (Primary)</th>
                      <th className="text-end" style={{ minWidth: 120 }}>Price (Base)</th>
                      <th className="text-end" style={{ minWidth: 130 }}>Price (Primary)</th>
                      <th className="text-end" style={{ minWidth: 100 }}>Discount</th>
                      <th className="text-end" style={{ minWidth: 130 }}>Line Total</th>
                    </tr>
                  </thead>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  newItems[i] = {
                                    ...cur,
                                    returnBaseQty: val,
                                    lineTotal: computeReturnLineTotal(cur, val, newPrimary),
                                  };
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  newItems[i] = {
                                    ...
                                    lineTotal: computeReturnLineTotal(cur, newBase, val),
                                  };
                                  setForm((p) => ({ ...p, items: newItems }));
                                }}
                              />
