...
const SalesInvoiceViewModal = ({
  ...
  // -----------------------------------------
  // Returned qty map (supports old + new field names)
  // -----------------------------------------
    ...
    embeddedReturns.forEach((ret) => {
      (ret.items || []).forEach((it) => {
        ...
        const baseReturned = Number(
          it.qtyReturnedBase ?? it.baseQtyReturned ?? it.baseQty ?? 0
        );

        const primaryReturned = Number(
          it.qtyReturnedPrimary ?? it.primaryQtyReturned ?? it.primaryQty ?? 0
        );
  ...
  // -----------------------------------------
  // Remaining totals map (authoritative backend truth)
  // -----------------------------------------
  ...
  return (
    ...
    >
      <style>{`
        ...
        .sales-invoice-view-modal .modal-content {
          height: 92vh;
          border-radius: 16px;
          overflow: hidden;
        }
        ...
        <div className="d-flex justify-content-between align-items-start w-100 gap-3">
          ...
          <div className="text-end me-3">
            <div
              ...
              style={{ background: "#f8fafc", minWidth: "230px" }}
            >
      ...
      <Modal.Body>
        ...
        <div className="d-flex flex-wrap gap-2 mb-3">
          <div className="siv-stat">
            <div className="k">Items</div>
            <div className="v">{itemCount}</div>
          </div>
          <div className="siv-stat">
            <div className="k">Returns</div>
            <div className="v">{returnCount}</div>
          </div>
          <div className="siv-stat">
            <div className="k">Payments</div>
            <div className="v">{paymentCount}</div>
          </div>
          <div className="siv-stat">
            <div className="k">Invoice Total</div>
            <div className="v">{formatCurrency(subTotal)}</div>
          </div>
          <div className="siv-stat">
            <div className="k">Paid</div>
            <div className="v">{formatCurrency(paidAmount)}</div>
          </div>
          <div className="siv-stat">
            <div className="k">Balance Due</div>
            <div className="v">{formatCurrency(dueAmount)}</div>
          </div>
        </div>
                  ...
                  payments.map((p, index) => (
                    <div key={index} className="siv-list-row">
                      ...
                      <div className="small text-muted text-center">
                        {formatDate(p.paymentDate || p.paymentId?.paymentDate)}
                      </div>

                      <div className="fw-bold text-end">
                        {formatCurrency(p.amount || p.paymentId?.amount)}
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="siv-side-scroll">
                {embeddedReturns.length === 0 ? (
                  ...
                  embeddedReturns.map((ret, index) => {
                    ...
                    const value =
                      Number(ret.totalReturnValue) ||
                      Number(ret.returnId?.totalReturnValue) ||
                      0;
